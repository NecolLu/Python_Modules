    ...
    def process_stream(self, stream: list[Any]) -> None:
        for item in stream:
            ...
            for proc in self.processors:
                # Polymorphism: We call validate() without knowing 
                # the specific class type of 'proc'
...
if __name__ == "__main__":
    ...
    stream_data = [
        ...
        True                          # Error: No processor handles bool
    ]
    ...
    # fulfills the "demonstrate output method" requirement
    print("\n>>> Consuming elements from processors:")
    for p in [num_proc, text_proc, log_proc]:
        try:
            status, data = p.output()
            print(f"[{p.__class__.__name__}] Output: {data} (Status: {status})")
