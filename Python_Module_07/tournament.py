...
from ex2 import NormalStrategy, AggressiveStrategy, DefensiveStrategy, BattleStrategy, InvalidStrategyError

def run_tournament(name: str, opponents: list[tuple[CreatureFactory, BattleStrategy]]) -> None:
    print(f"Tournament {name}")
    
    # Format visual representation for the example match layout
    opp_names = [f"({fact.__class__.__name__.replace('CreatureFactory', '').replace('Factory', '')}+{strat.__class__.__name__.replace('Strategy', '')})" for fact, strat in opponents]
    print(f"[ {', '.join(opp_names)} ]")
    print("*** Tournament ***")
    print(f"{len(opponents)} opponents involved")
    
    # To avoid modifying state mid-loop, we pre-generate instances for the round robin
...
if __name__ == "__main__":
    ...
    run_tournament("2 (multiple)", [(aqua, normal), (healing, defensive), (transform, aggressive)])